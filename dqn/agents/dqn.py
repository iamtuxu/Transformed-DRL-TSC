...
class DqnAgent:
    ...
    def select_action(self, state, steps_done, invalid_action):
        ...
        if self.mode == 'train':
            ...
            # if sum(decrease_state) < 0.2 or any(element < 0.1 for element in decrease_state):
            #     return decrease_state.index(max(decrease_state))
            with torch.no_grad():
                _, sorted_indices = torch.sort(self.policy_net(state), descending=True)
                if invalid_action:
                    return sorted_indices[1]
    ...
    def learn(self):
        if self.mode == 'train':
            if self.replay.steps_done <= 10000: # was 10000
                return
            loss_fn = nn.MSELoss()
            if self.use_sgd:
                optimizer = torch.optim.SGD(self.policy_net.parameters(), lr=0.0001)
            else:
    ...
    def learn_gamma(self):
        ...
        transitions = self.replay.sample(self.batch_size) #从replay buffer中选择transitions
        batch = Transition(*zip(*transitions)) #解包运算
        state_batch = torch.cat(batch.state) #将状态连接到一起，按列组合
        action_batch = torch.cat(batch.action).view(self.batch_size, 1) #将动作连接到一起，按列组合
        next_state_batch = torch.cat(batch.next_state)
        reward_batch = torch.cat(batch.reward).view(self.batch_size, 1) #将奖励连接到一起，按列组合
        state_action_values = self.policy_net(state_batch).gather(1, action_batch)
        with torch.no_grad():
            argmax_action = self.policy_net(next_state_batch).max(1)[1].view(self.batch_size, 1)
            q_max = self.target_net(next_state_batch).gather(1, argmax_action)
            expected_state_action_values = reward_batch + self.fixed_gamma * q_max #计算期望的状态动作值
